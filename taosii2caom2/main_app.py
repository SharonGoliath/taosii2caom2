...
class TAOSIIMapping(TelescopeMapping):
    def __init__(self, storage_name, h5file):
        super().__init__(storage_name, headers=None)
    ...
    def accumulate_blueprint(self, bp, application=None):
        ...
        bp.set(
            'Observation.instrument.keywords', (['//header/run/exptype'], None)
        )
        bp.set('Observation.proposal.id', (['//header/run/run_seq'], None))
        bp.set('Observation.proposal.pi', (['//header/run/observer'], None))
        ...
        bp.set(
            'Observation.target.name', (['//header/pointing/field_id'], None)
        )
        # bp.set('Observation.target.type', (['//header/run/imgtype'], None))
        bp.set('Observation.target.type', 'object')
        bp.set(
            'Observation.target.targetID', (['//header/object/obj_type'], None)
        )
        bp.set(
        ...
        bp.set('Chunk.energy.axis.range.start.val', 400)
    ...
    def _get_target_position(self):
        obj_ra = self._lookup('header/object/obj_ra')
        obj_dec = self._lookup('header/object/obj_dec')
        if obj_ra is None or obj_dec is None:
            ra = None
            dec = None
        else:
            ra, dec = build_ra_dec_as_deg(
                obj_ra.decode('utf-8'), obj_dec.decode('utf-8')
            )
        return ra, dec

    def get_time_axis_range_end(self, ext):
        return self._lookup('header/timeseries/numepochs')
    ...
    def _lookup(self, things):
        ...
        if result is None:
            ...
            self._logger.error(f'Found {result} for {things}')
        return result

    def _update_artifact(self, artifact, clients):
        ...
        for part in artifact.parts.values():
            for chunk in part.chunks:
                if (
                    ...
                    # blueprint can't figure that out
...
class TAOSIIName(StorageName):
    ...
    def __init__(self, file_name=None, source_names=None):
        super_source_names = (
            [file_name] if source_names is None else source_names
        )
        super().__init__(file_name=file_name, source_names=super_source_names)
...
class TAOSII2caom2Visitor(Fits2caom2Visitor):
    ...
    def _get_parser(self, headers, blueprint, uri):
        ...
        return Hdf5Parser(blueprint, uri, self._h5_file)
